...
class _BaseOverlap:
    ...
    def __init__(
        ...
        if mask_trace_profile is None:
            ...
            mask_trace_profile = np.array(
                [np.zeros(self.data_shape, dtype=bool) for _ in orders]
            )
        ...
        if wave_grid is None:
            ...
            if not is_sorted:
                log.warning(
                    "`wave_grid` is not strictly increasing. It will be sorted and unique."
                )
        ...
        if np.any(good_pixels_in_order < min_good_pixels):
            raise MaskOverlapError(
                "At least one order has no valid pixels (mask_trace_profile and mask_wave do not overlap)"
            )
    ...
    def update_trace_profile(self, trace_profile):
        ...
        self.trace_profile = [
            trace_profile_n.astype(dtype).copy() for trace_profile_n in trace_profile
        ]
    ...
    def _get_masks(self, global_mask):
        ...
        is_contaminated = np.array(
            [tr_profile_ord > threshold for tr_profile_ord in trace_profile]
        )
    ...
    def get_pixel_mapping(self, i_order, same=False, error=True, quick=False):
        ...
        if same:
            ...
            trace_profile_n, throughput_n, kernel_n, weights_n, i_bnds = (
    ...
    def _solve(matrix, result):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                log.info(
                    "ATOCA matrix solve failed with spsolve. Retrying with least-squares."
                )
